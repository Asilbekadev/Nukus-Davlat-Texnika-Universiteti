"""
🎓 NDTU Test Bot - Kompyuter Tarmoqlari 2 bo'limli versiya
network_part1.json va network_part2.json fayllaridan foydalanadi
"""
...
async def start_test(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Agar network_part1 yoki network_part2 bo'lsa
    if len(query.data.split('_')) > 3:
        subject = query.data.split('_', 2)[2]  # "network_part1" yoki "network_part2"
...
async def send_question(query, user_id):
    ...
    try:
        # ⭐ YANGI: network_part1 va network_part2 uchun alohida fayllar
...
async def check_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if session.subject == 'network_part1':
            with open('network_part1.json', 'r', encoding='utf-8') as f:
                question = json.load(f)[session.current_question]
        elif session.subject == 'network_part2':
            with open('network_part2.json', 'r', encoding='utf-8') as f:
                question = json.load(f)[session.current_question]
        else:
            with open('questions.json', 'r', encoding='utf-8') as f:
                question = json.load(f)[session.subject][session.current_question]
    except:
...
async def finish_test(query, user_id):
    ...
    keyboard = [
        [InlineKeyboardButton("🔄 Qayta urinish", callback_data=f"start_test_{session.subject}")],
...
def main():
    ...
    print("✅ NDTU Test Bot tayyor!")
    print("🎓 Bot ishga tushdi...")
    print("📘 Kompyuter Tarmoqlari 2 bo'limli versiya")
